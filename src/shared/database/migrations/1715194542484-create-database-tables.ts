...
export class CreateDatabaseTables1715194542484 implements MigrationInterface {
  name = 'CreateDatabaseTables1715194542484';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`tasks\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` timestamp(6) NULL, \`description\` varchar(200) NOT NULL, \`status\` enum ('PENDING', 'COMPLETED') NOT NULL, \`due_date\` timestamp NOT NULL, \`to_do_list_id\` bigint NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
